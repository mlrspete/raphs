...
declare
  ...
  v_effective_entries_close_at timestamptz;
  v_entries_close_base timestamptz;
...
begin
  ...
  if v_campaign.entry_limit is not null and v_existing_entry_count >= v_campaign.entry_limit then
    raise exception 'Campaign entries are sold out; manual review required';
  end if;

  if v_campaign.entry_limit is not null and v_existing_entry_count + p_quantity > v_campaign.entry_limit then
    raise exception 'Selected Daypass quantity exceeds remaining promotion entries; manual review required';
  end if;

  v_entries_close_base := v_campaign.entries_close_at;
  v_effective_entries_close_at := v_entries_close_base;

  if v_entries_close_base is not null and v_now >= v_entries_close_base then
    if v_campaign.entry_limit is null or v_existing_entry_count < v_campaign.entry_limit then
      v_effective_entries_close_at :=
        v_entries_close_base
        + ((floor(extract(epoch from (v_now - v_entries_close_base)) / 86400)::integer + 1) * interval '1 day');
    end if;
  end if;

  if v_effective_entries_close_at is not null and v_now > v_effective_entries_close_at then
